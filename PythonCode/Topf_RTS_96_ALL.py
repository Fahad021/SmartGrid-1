...
#PV-SWING busses :
def PVS_rule(model):
    return [i['cdfNum'] for i in n if i['busType'] in[2,3]]
...
def InitConnActive(model,node):
    return [
        ...
        if j['fromBus'] == node
        and j['TR'] != 0
        and j['cdfNum'] in model.branches
    ]
...
def populateNodes_13(model,node):
    for j in g:
        return j['genVolt'] if j['locBus']==node else 1
...
def populateGenerators_7(model,gen):
    for j in g:
        if j['genID']==gen:
            return 1e9 if model.busType[j['locBus']]==3 else j['genQMAX']
...
def populateGenerators_9(model,gen):
    for j in g:
        if j['genID']==gen:
            return 1e9 if model.busType[j['locBus']]==3 else j['genPg']*1.5
...
def populateBranches_12(model,branch):
    for i in k:
        if i['cdfNum']==branch:
            return 999 if i['X'] ==0 else i['R'] /(i['X']**2+i['R']**2)
...
def populateBranches_11(model,branch):
    for i in k:
        if i['cdfNum']==branch:
            return 999 if i['X'] ==0 else model.BranchG[branch] *0.5
...
def populateBranches_10(model,branch):
    for i in k:
        if i['cdfNum']==branch:
            return 999 if i['X'] ==0 else i['R'] /(i['X']**2+i['R']**2) *1.5
